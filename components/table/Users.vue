...
async function removeMany() {
    ...
    try {
        ...
        await repo.destroyMany(selected.value.map((item: User) => item.id));
...
<template>
    <div>
        ...
        <ModalEditModel v-model="editRow"
            ...
            <template #create-title>Новый пользователь</template>
            <template #edit-title="{state}">Пользователь #{{ state.id }}</template>
